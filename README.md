...
1. Explain the difference between `RDBMS` and `SQL`.
    ...
    RDBMS is the most widely used type of database management system and is based on the relational model which sees data stored in tables. SQL is the standard language used to manage databases and the data within them is supported by most RDBMS.
    
1. Why do tables need a `primary key`?

    Tables require a primary key in order to uniquely identify each record in the table and reference it when required. Because it is unique, it cannot be changed and a deleted id will not be reassigned.
...
1. What do we need in order to have a _many to many_ relationship between two tables.
    ...
    In order to have a many to many relationship between two tables, a third table is required. This table will hold the foreign keys that reference the primary key on the tables included in that relationship.
